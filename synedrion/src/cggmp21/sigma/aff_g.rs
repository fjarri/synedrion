...
impl<P: SchemeParams> AffGProof<P> {
    ...
    pub fn new(
        ...
        let beta = SecretSigned::random_bounded_bits(rng, P::LP_BOUND + P::EPS_BOUND);
        ...
        let m = SecretSigned::random_bounded_bits_scaled(rng, P::L_BOUND, hat_cap_n);
        let delta = SecretSigned::random_bounded_bits_scaled(rng, P::L_BOUND + P::EPS_BOUND, hat_cap_n);
...
mod tests {
    ...
    fn prove_and_verify() {
        ...
        let y = SecretSigned::random_bounded_bits(&mut OsRng, Params::LP_BOUND);

        let rho = Randomizer::random(&mut OsRng, pk0);
        let rho_y = Randomizer::random(&mut OsRng, pk1);
        let secret = SecretSigned::random_bounded_bits(&mut OsRng, Params::L_BOUND);
